...
The survey is in 6 languages, two of which (Bengali, Vietnamese) use scripts not
fluently readable by anyone who worked on this project. Manually reading translated
text and re-typing it into another document carries a real risk of silent errors —
skipped characters, wrong diacritics — that would be difficult to catch by proofreading
alone.

To avoid this, no translated text in this project's documentation was ever manually
transcribed. Instead, the `translation-pipeline/` scripts extract the survey's actual
JavaScript and execute it for real (via Node.js), so the *live interpreter* resolves
each language's exact question text and options — the same resolution the survey itself
performs in a respondent's browser. Every other document (the Word build guide, the
xlsx question reference) was generated from that resolved output, not from a human
reading and re-typing the HTML by eye. Full explanation and reusable scripts are in
`translation-pipeline/README.md`.
